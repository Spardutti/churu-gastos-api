...
class CurrentMonthAccountBalanceAPIView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"data": { "account_balances": budget}
        }, status=status.HTTP_200_OK)


    def total_balance(self, balances):
        total = 0
    ...
    def budget(self, year, month):
        ...
        total_balance = self.total_balance(AccountBudget.objects.filter(user=self.request.user, date__lte=end_date, date__gte=start_date))
        ...
        return total_balance - total_expenses - total_unique_expenses