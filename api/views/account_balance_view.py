...
class AccountBalanceAPIView(APIView):
    ...
    def patch(self, request, pk=None):
        account_balance = AccountBalance.objects.get(pk=pk, user=request.user)
        serializer = AccountBalanceSerializer(account_balance, data=request.data, partial=True)
        if(serializer.is_valid()):
            serializer.save()
            return Response({"data": serializer.data}, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def filter_by_months(self, year, month, user):
        ...
        return AccountBalance.objects.filter(
            user=user,
